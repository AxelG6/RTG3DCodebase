...
// Time uniform for animating the vortex
uniform float time;
...
void main(void) {
    ...
    // --- Vortex Effect on Texture Coordinates ---
    // Define the center of the vortex in UV space (e.g., middle of texture)
    vec2 vortexCenter = vec2(0.5, 0.5);

    // Compute the offset from the vortex center
    vec2 offset = inputFragment.texCoord - vortexCenter;

    // Compute the distance from the vortex center
    float distance = length(offset);

    // Compute the vortex rotation angle based on time and distance
    float angle = time * 2.0 - distance * 10.0; // Adjust speed and intensity

    // Apply the rotation to the texture coordinates
    mat2 rotation = mat2(cos(angle), -sin(angle),
                         sin(angle),  cos(angle));
    vec2 rotatedTexCoord = rotation * offset + vortexCenter;

    // Sample the surface color using the distorted texture coordinates
    vec4 surfaceColour = texture(sampleTexture, rotatedTexCoord);
